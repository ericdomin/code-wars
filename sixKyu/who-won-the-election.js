...
Usually, we vote for a candidate, i.e. a set of eligible candidates is given. This is done by casting a ballot into a 
ballot-box. After that, it must be counted how many ballots (= votes) a candidate got.

A candidate will win this election if he has the absolute majority.

Your Task
Return the name of the winner. If there is no winner, return null (in Java and JavaScript), None (in Python), nil (in Ruby),
 or * in C.
Task Description
There are no given candidates. An elector can vote for anyone. Each ballot contains only one name and represents one vote for this
 name. A name is an arbitrary string, e.g. "A", "B", or "XJDHD".
...
function getWinner(listOfBallots) {
    //Your code here
    const count = listOfBallots.reduce((obj,item)=>{
      if( !obj[item] )
      obj[item]=0;
      obj[item]++;
      return obj
  ...
  let majority = Math.max(listOfBallots.length/2)
  let maxo = Object.keys(count).find(key=>count[key]==max)
  let mino =Object.keys(count).find(key=>count[key]==min)
  if(new Set (Object.keys(count)).size==1) return listOfBallots[0]
  else if(listOfBallots.length==1)return listOfBallots[0];
  else return max==min || max<=majority ? null : maxo;
