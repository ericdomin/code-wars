/*JavaScript's indexOf does not work with arrays as input. This is because [1,2] === [1,2] 
will return false in JavaScript. Many other languages have similar quirks.

However, sometimes it is useful to search for an Array. Write a function that looks for an array within a 
two-dimensional array and returns the index of the first matching element. If there is no match, your function
 should return -1.
...
var searchArray = function (arrayToSearch, query) {
    ...
    if(query.length>2)throw new Error("Error");
    for(let i = 0; i <arrayToSearch.length;i++){
      if(!Array.isArray(arrayToSearch[i]) || arrayToSearch[i].length !=2)throw new Error('error')
